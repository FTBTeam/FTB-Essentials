    ...
    public static int recording(ServerPlayer player) {
        return FTBEPlayerData.getOrCreate(player).map(data -> {
            ...
            if (data.getRecording() == FTBEPlayerData.RecordingStatus.RECORDING) {
                ...
                player.level().getServer().getPlayerList().broadcastSystemMessage(player.getDisplayName().copy().withStyle(ChatFormatting.YELLOW).append(" ").append(Component.translatable("ftbessentials.chat.status.stop_record")), false);
            }
    ...
    public static int streaming(ServerPlayer player) {
        return FTBEPlayerData.getOrCreate(player).map(data -> {
            ...
            if (data.getRecording() == FTBEPlayerData.RecordingStatus.STREAMING) {
                ...
                player.level().getServer().getPlayerList().broadcastSystemMessage(player.getDisplayName().copy().withStyle(ChatFormatting.YELLOW).append(" ").append(Component.translatable("ftbessentials.chat.status.stop_stream")), false);
            }
